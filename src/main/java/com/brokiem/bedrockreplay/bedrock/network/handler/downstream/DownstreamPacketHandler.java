 ...
 */
 ...
 public class DownstreamPacketHandler implements BedrockPacketHandler {
     ...
     public PacketSignal handle(LevelChunkPacket packet) {
         // Retain the packet to prevent premature release
         ...
         // Release the packet after storing it in the cache
     ...
     public PacketSignal handle(ClientCacheMissResponsePacket packet) {
         ...
         // Process the packet (e.g., forward or cache it)
         player.getSession().sendPacket(packet);
     
         // Release the packet after processing
         packet.release();
         return PacketSignal.HANDLED;
     }
 }