 ...
 public class UpstreamPacketHandler implements BedrockPacketHandler {
     ...
     public PacketSignal handle(LoginPacket packet) {
         try {
             ...
             // Log the start of the login handling process
             OutputWindow.print("Handling LoginPacket for player: " + player.getPlayerInfo().username());
 
             // Log the extra data from the packet
             String extraData = packet.getExtra();
             OutputWindow.print("Received extraData: " + extraData);
 
             // Retrieve downstream server details and log them
             ...
             OutputWindow.print("Connecting to downstream server at " + downstreamAddress + ":" + downstreamPort);
 
             // Decode and set the player's skin data
             String decodedSkinData = new String(Base64.getUrlDecoder().decode(extraData.split("\\.")[1]));
             OutputWindow.print("Decoded skin data: " + decodedSkinData);
             player.setSkinData(JSONObject.parseObject(decodedSkinData));
 
             // Send login success packet and log the action
             PlayStatusPacket playStatusPacket = new PlayStatusPacket();
             playStatusPacket.setStatus(PlayStatusPacket.Status.LOGIN_SUCCESS);
             player.getSession().sendPacket(playStatusPacket);
             OutputWindow.print("Sent PlayStatusPacket with status: LOGIN_SUCCESS");
 
             // Connect to the downstream server and log the connection attempt
             player.connect(downstreamAddress, downstreamPort);
             OutputWindow.print("Player connected to downstream server successfully.");
 
             return PacketSignal.HANDLED;
         } catch (Exception e) {
             // Log any exceptions that occur during the login process
             OutputWindow.print("Error handling LoginPacket: " + e.getMessage());
             e.printStackTrace();
